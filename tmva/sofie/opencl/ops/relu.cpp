...
int main()
{
   ...
   const char *kernelSource = "__kernel void vecAdd(__global float* A, __global float* C) {"
   ...
   cl_event transfer1;
   err = clEnqueueWriteBuffer(queue, bufferA, CL_TRUE, 0, VECTOR_SIZE * sizeof(float), A, 0, NULL, &transfer1);
   if (err != CL_SUCCESS) {
